...
NODE_META: dict[str, dict[str, Any]] = {
    ...
    "intent_router": {"title": "意图路由", "implemented": "live", "execution_note": "启发式规则基线：按问题措辞识别 procedural/comparison/risk_adjacent/factual；不是训练分类器。", "teach": {"what": "按问题意图选择不同检索分支。当前是可运行的规则基线，后续可替换为分类模型。", "tune": "规则在 executor.py 中；替换分类器时保持相同 query 输出契约。", "pitfalls": "规则基线不是生产级语义分类器，必须用 Intent Macro-F1 评测后再替换。"}},
    ...
    "evidence_grade": {"title": "证据评分", "implemented": "live", "execution_note": "启发式证据充分性判断：按候选数量与 top score 决定 sufficient/insufficient。", "teach": {"what": "判断证据是否足以回答，不足时交给有界纠错检索。", "tune": "min_evidence/min_top_score 来自节点 config。", "pitfalls": "这是确定性基线，不等同于 NLI 或 LLM 证据判定，需用拒答与 Citation Support Eval 校准。"}},
...
CONFIG_SCHEMAS: dict[str, list[dict[str, Any]]] = {
    ...
    "dense_retrieve": [
        ...
        {"key": "model_ref", "label": "Embedding 模型", "type": "model", "model_kind": "embedding", "effective": True, "help": "查询侧模型必须与索引侧兼容"},
    ],
    "sparse_retrieve": [
        {"key": "top_k", "label": "Top K", "type": "number", "min": 1, "max": 50, "step": 1, "effective": True, "help": "内置 BM25 候选数量"},
        {"key": "k1", "label": "BM25 k1", "type": "number", "min": 0, "max": 4, "step": 0.05, "effective": True},
        {"key": "b", "label": "BM25 b", "type": "number", "min": 0, "max": 1, "step": 0.05, "effective": True},
    ],
    "rrf_fusion": [{"key": "k", "label": "RRF k", "type": "number", "min": 1, "max": 200, "step": 1, "effective": True, "help": "多路候选真实融合"}],
    "reranker": [
        {"key": "model_ref", "label": "Reranker 模型", "type": "model", "model_kind": "reranker", "effective": True, "help": "绑定可达 /rerank 端点时真实重排，否则如实直通"},
        {"key": "candidate_k", "label": "精排候选数", "type": "number", "min": 1, "max": 100, "step": 1, "effective": True},
        {"key": "final_k", "label": "输出条数", "type": "number", "min": 1, "max": 20, "step": 1, "effective": True},
    ],
    "context_builder": [
        {"key": "token_budget", "label": "Token 预算", "type": "number", "min": 500, "max": 16000, "step": 100, "effective": True, "help": "真实控制上下文字符预算"},
        {"key": "mmr_lambda", "label": "MMR λ", "type": "number", "min": 0, "max": 1, "step": 0.05, "effective": False, "help": "当前尚未启用 MMR"},
    ],
    "evidence_grade": [
        {"key": "min_evidence", "label": "最少证据数", "type": "number", "min": 0, "max": 20, "step": 1, "effective": True},
        {"key": "min_top_score", "label": "最低 Top Score", "type": "number", "min": 0, "max": 1, "step": 0.05, "effective": True},
    ],
    "llm_generate": [
    ...
    "metadata_filter": [{"key": "on_empty", "label": "过滤为空时", "type": "select", "options": ["fallback_once", "fail"], "effective": True, "help": "空结果时是否保留未过滤候选池"}],
    "bounded_corrective": [{"key": "max_retries", "label": "最大重试", "type": "number", "min": 0, "max": 2, "step": 1, "effective": True, "help": "执行器硬上限为 2"}],
    "cache": [{"key": "ttl_seconds", "label": "TTL（秒）", "type": "number", "min": 0, "max": 86400, "step": 30, "effective": True, "help": "进程内 TTL；多副本需 Redis/网关"}],
    "rate_limit": [{"key": "requests_per_minute", "label": "每分钟请求数", "type": "number", "min": 1, "max": 6000, "step": 1, "effective": True, "help": "单进程 Recipe 限流；多副本需网关"}],
