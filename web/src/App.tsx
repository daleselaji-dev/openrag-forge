...
type Scenario = { scenario_id: string; title: string; business_problem: string; recipe_id: string; sample_question: string; data_requirements: string[]; trace_expectation: string[]; source_urls: string[] }
...
function App() {
  ...
  const upload = async (file: File) => {
    ...
    try {
      ...
      const result = await api<{ document: Document; blocks: number; chunks: number; route: { route: string; confidence: number; reason_codes: string[] }; index?: { status: string; embedding_model_id?: string } }>(`/api/v1/knowledge-bases/${kbId}/documents?${query.toString()}`, { method: 'POST', body: form })
  ...
  return <div className="shell">
    ...
    <main>
      ...
      <section className="panel scenario-panel"><div className="panel-head"><div><span className="eyebrow">REAL SCENARIO GALLERY</span><h2>真实企业场景示范</h2></div><span className="pill">问题 → Recipe → Trace</span></div><p className="muted">每个示范都声明业务问题、所需资料、默认装配和应观察的 Trace。点击后会加载示例问题；运行前仍需导入对应知识库文档。</p><div className="scenario-grid">{scenarios.map((scenario) => <article className="scenario-card" key={scenario.scenario_id}><span className="eyebrow">{scenario.recipe_id}</span><h3>{scenario.title}</h3><p>{scenario.business_problem}</p><small>资料：{scenario.data_requirements.join(' · ')}</small><code>{scenario.sample_question}</code><button className="ghost" onClick={() => useScenario(scenario)}>加载示范问题</button></article>)}</div></section>
